...
input_schema: the exact shape of arguments the tool expects
              (reuses Week 2 Day 2's schema concept — same idea,
...
This is the same instinct as Week 2 Day 3's "versions, not scattered copies" — one source of truth that multiple call sites read from, instead of copies that can silently diverge.
...
Week 3 Day 1 adds: the model can now ask YOUR CODE to do something
