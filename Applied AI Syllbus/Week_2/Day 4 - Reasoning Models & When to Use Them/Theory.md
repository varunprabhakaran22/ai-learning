...
classify: simple / complex / creative / analytical
      ↓
   simple      → standard model, factual preset (Week 1 Day 3)
...
**This directly reuses Week 1 Day 3's `ParameterPreset` system** — reasoning-model-or-not is now another dimension of the same routing decision that temperature/top-p already was. The showcase task (`ModelRouter`) is the piece of code that makes this decision automatically instead of you manually picking a model every time.
...
- Tokenizer differences: different model families can tokenize the
  same text differently (Week 1 Day 4). Re-count tokens against the
...
- Context window differences: if the new model has a smaller window,
  ...
  window (Week 1 Day 4) against the new budget, not the old one.
